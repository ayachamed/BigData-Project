...
The search queries were carefully selected to cover various aspects of the conflict, and we configured the collector to retrieve up to 50 videos for each query to ensure a sufficient dataset size:
...
def search_videos(self, query, max_results=50, published_after=None, published_before=None):
    ...
    params = {
        ...
        'key': self.api_key,
        'order': 'relevance', # Sort by relevance to get top videos
...
\subsection{5.2 Top Videos by Engagement}
...
\subsection{5.3 Temporal Evolution}
...
\subsection{5.4 Keyword Analysis}
...
\subsection{5.5 Query Distribution}
...
\subsection{5.6 Engagement Metrics}
...
\begin{itemize}
    \item feeling Analysis: Integrating Natural Language Processing (NLP) models (e.g., VADER or BERT) to classify the feeling of millions of user comments.
